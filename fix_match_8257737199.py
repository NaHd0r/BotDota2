...
"""
Script pour insérer directement les données du match 8257737199 dans le cache live.
...
def fix_match_in_live_cache():
    """Insère directement les données du match 8257737199 dans le cache live"""
    # Charger les caches
    ...
    series_id = "8257737199"  # Utiliser l'ID du match comme ID de série
    ...
    formatted_match = {
        ...
        'duration': match_data.get('duration', "41:47"),
        'duration_formatted': match_data.get('duration', "41:47"),
        'radiant_networth': 0,
        'dire_networth': 0,
        'networth_difference': 0,
        'draft_phase': False,
        'match_outcome': 2 if match_data.get('winner') == 'dire' else 1,  # 1=Radiant win, 2=Dire win
        'game_number': 1,
        'timestamp': match_data.get('timestamp', 1744872937),
        'match_state': {
            'phase': 'finished',
            'winner': match_data.get('winner', 'dire')
        }
    }
    ...
    if series_id not in live_cache:
        live_cache[series_id] = {
            ...
            'dire_id': "",
            'league_id': league_id,
            'radiant_score': 0 if match_data.get('winner') == 'dire' else 1,
            'dire_score': 1 if match_data.get('winner') == 'dire' else 0,
            'matches': {},
            'last_update_time': 1744872937,
            'created_at': 1744872937
        }
...
if __name__ == "__main__":
    if fix_match_in_live_cache():
        print("Le match 8257737199 a été ajouté au cache live.")
    else:
        print("Erreur lors de l'ajout du match 8257737199 au cache live.")